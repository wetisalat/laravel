  ...
  <b-card>
    ...
    <b-form>
          ...
          <b-form-group label="Old Password" label-for="account-old-password">
            <b-input-group class="input-group-merge">
              <b-form-input id="account-old-password" v-model="passwordValueOld" name="old-password"
                :type="passwordFieldTypeOld" placeholder="Old Password" />
      ...
      <b-row>
          ...
          <b-form-group label-for="account-retype-new-password" label="Retype New Password">
            <b-input-group class="input-group-merge">
              <b-form-input id="account-retype-new-password" v-model="RetypePassword" :type="passwordFieldTypeRetype"
                name="retype-password" placeholder="New Password" />
              <b-input-group-append is-text>
        ...
        <!-- buttons -->
        <b-col cols="12">
          <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mt-1 mr-1" @click="SaveChange">
            Save changes
          </b-button>
...
export default {
  ...
  methods: {
    ...
    SaveChange() {
      ...
      axios.post("api/client/setting-password/save_change", data)
        .then(res => {
          if (res.data.success == true) {
            this.PopUpNotification(res.data.message, 'success')
            this.reset() 
          }else{
            this.PopUpNotification(res.data.message, 'error')
          }
        })
        .catch(err => {
