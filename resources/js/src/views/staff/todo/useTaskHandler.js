...
export default function useTaskHandler(props, emit) {
    ...
    let assigneeOptions = props.assignee;
    const resetTaskLocal = () => {
        taskLocal.value = JSON.parse(JSON.stringify(props.task.value));
	taskLocal.value.isCompleted = taskLocal.value.status!=undefined?taskLocal.value.status.includes('Completed'):false
        taskLocal.value.isImportant = taskLocal.value.status!=undefined?taskLocal.value.status.includes('Important'):false
    };
    watch(props.task, () => {
        resetTaskLocal();
    });
